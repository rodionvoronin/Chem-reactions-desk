 ...
 */
export function TaskPalette({ reagents, tubeSelected, overBudget, onReagentClick, onClearTube }: Props) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        padding: '11px 13px 8px', borderBottom: '1px solid #f0f0f0',
        fontSize: 11, fontWeight: 700, color: '#888', letterSpacing: 0.5,
      }}>
        ДОСТУПНЫЕ РЕАГЕНТЫ
      </div>

      <div style={{ padding: '9px 10px', display: 'flex', flexDirection: 'column', gap: 4, overflowY: 'auto' }}>
        {list.map((id) => {
          ...
          return (
            <button
              ...
              style={{
                ...
                fontSize: 13, fontWeight: 500, fontFamily: FONT, textAlign: 'left',
        ...
        {heat && (
          <button
            ...
            style={{
              marginTop: 2, padding: '9px 0', border: 'none', borderRadius: 8,
        ...
        {list.length === 0 && !heat && (
          <p style={{ margin: 0, fontSize: 11, color: '#bbb', lineHeight: 1.4 }}>
      ...
      <div style={{ padding: '0 10px 11px' }}>
        ...
        <button
          ...
          style={{
            width: '100%', border: '1.5px solid #FFCCBC', borderRadius: 7, padding: '7px 10px',
